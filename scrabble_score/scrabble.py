"""Scrabble score game for a given word."""
import time
...
def is_valid_word(word):
    ...
    valid_words = {"scrabble", "game", "jahan",
                   "software", "tools", "process", "cabbage"}
    return word.lower() in valid_words


def game_round(timer_seconds, req_length):
    """Simulate a game round with a timer and word length requirement."""
    start_time = time.time()
    prompt = f"Enter a word with {req_length} letters (or 'quit' to exit): "
    word = input(prompt).strip()

    if word.lower() == "quit":
        return (-1, "Game ended. Exiting...")
    ...
    if len(word) != req_length:
        return (
            0, f"Invalid length! The word should be {req_length} letters."
            )
